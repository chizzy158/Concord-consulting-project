...
/* ──────────────────────────────────────
   SECURITY MIDDLEWARE
   CSP configured for CloudFront HTTPS
────────────────────────────────────── */
app.use(helmet({
  // Disable headers that cause HTTP→HTTPS upgrade issues
  ...
  contentSecurityPolicy: {
    directives: {
      ...
      scriptSrc:     ["'self'", "'unsafe-inline'", "'unsafe-eval'", "https:", "http:", "cdn-cgi"],
      scriptSrcAttr: ["'self'", "'unsafe-inline'", "'unsafe-eval'"],
      scriptSrcElem: ["'self'", "'unsafe-inline'", "https:", "http:", "fonts.googleapis.com", "cdn-cgi"],
      styleSrc:      ["'self'", "'unsafe-inline'", "https:", "fonts.googleapis.com"],
      fontSrc:       ["'self'", "https:", "data:", "fonts.gstatic.com"],
      imgSrc:        ["'self'", "data:", "https:", "http:"],
      connectSrc:    ["'self'", "https:", "http:"],
      frameSrc:      ["'self'", "https://www.google.com", "https:"],
    }
  }
}));

// Trust proxy — needed when behind CloudFront
app.set('trust proxy', 1);

app.use(express.json({ limit: '10kb' }));
app.use(express.urlencoded({ extended: true, limit: '10kb' }));

// Request logging — logs every request with method, path, status and IP
...
/* ──────────────────────────────────────
   INIT DATABASE TABLE
   Retries every 10 seconds if DB not ready
────────────────────────────────────── */
let dbReady = false;
